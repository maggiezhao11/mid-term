...
const myResources = (db) => {
  router.get("/", (req, res) => { // ***"/:id" we only need to put :id in the routes path for params not for cookies
    ...
    db.query(`SELECT * FROM resources WHERE resources.owner_id = $1`, [userID]) // the name after params is the same one after the /:
      .then(data => {
        ...
        db.query(`SELECT * FROM resources JOIN user_likes ON resources.id=user_likes.resource_id WHERE user_likes.owner_id = $1;`, [userID])
      .then(result => {
